...
if (mysqli_num_rows($clientes) > 0) {
    foreach($clientes as $cliente) {
        ...
        <?php
        echo" <td>
        <button class='btn btn-primary btn-sm' type='submit' name='alterar'>Alterar</button>&nbsp&nbsp&nbsp
        <button class='btn btn-danger btn-sm' type='submit' name='Excluir' value='".$cliente['id']."'>Excluir</button>
        </td>";
        ?>
        </form>
        <?php
